...
var websocket = null,
    actionUUID = "com.ulanzi.time.action",
    imageIndex = 0,
    key = null,
    keepLive = true,
    isQT = navigator.appVersion.includes('QtWebEngine');

connectUlanziDeckSocket();

function connectUlanziDeckSocket() {
    console.log("connectElgatoStreamDeckSocket " + actionUUID)

    keepLive = true
    websocket = new WebSocket('ws://127.0.0.1:' + 3906);
...
function websocketOnOpen() {
    function registerPlugin(inPluginUUID) {
        let json = {
            ...
            cmd: "connected",
            uuid: inPluginUUID };
        console.log("registerPlugin " + JSON.stringify(json))
        websocket.send(JSON.stringify(json));
    }
    registerPlugin(actionUUID)
}

function websocketOnMessage(evt) {
    // Received message from UlanziDeck
    var jsonObj = JSON.parse(evt.data);
    console.log(`[websocketOnMessage]:${evt.data}`);
    ...
    let isResponse = code !== undefined

    if (event === 'paramfromapp') {
        ...
        sendStateToPlugin(JSON.stringify(json))
    } else if (event === 'paramfromplugin') {
        console.log("paramfromplugin: " + evt.data);
...
function sendStateToPlugin(jsonString) {
    if (websocket && (websocket.readyState === 1)) {
